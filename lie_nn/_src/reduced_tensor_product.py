...
# from lie_nn import TabulatedIrrep, SumRep, MulRep, Rep
# import numpy as np
# import lie_nn._src.finite.sn as sn
...
#                 muls[-1] += mul
#         return IrrepsArray(
#             list=list, irreps=tuple(MulRep(mul, irrep) for mul, irrep in zip(muls, irreps))
#         )
...
# def _to_reducedrep(irreps) -> SumRep:
#     if isinstance(irreps, TabulatedIrrep):
#         irreps = MulRep(1, irreps)
#     if isinstance(irreps, MulRep):
#         irreps = SumRep.from_irreps([irreps])
#     assert isinstance(irreps, SumRep)
#     return irreps


# def reduced_tensor_product_basis(
#     formula_or_irreps_list: Union[str, List[SumRep]],
#     *,
#     epsilon: float = 1e-5,
#     **irreps_dict,
# ) -> RepArray:
...
#         return RepArray(SumRep.from_irreps(out.irreps), out.array, out.list)
...
#     irreps_tuple: Tuple[SumRep, ...],
#     perm_repr: FrozenSet[Tuple[int, Tuple[int, ...]]],
#     epsilon: float,
# ) -> IrrepsArray:
#     dims = tuple(irps.dim for irps in irreps_tuple)

#     def get_initial_basis(reduced_rep: SumRep, i: int) -> List[np.ndarray]:
#         x = np.reshape(
...
#         x_list = []
#         cursor = 0
#         for mul_ir in reduced_rep.reps:
...
#         (
#             frozenset({i}),
#             IrrepsArray(list=get_initial_basis(reduced_rep, i), irreps=reduced_rep.reps),
#         )
...
#     while True:
#         n = len(perm_repr)
#         perm_repr = perm_repr.union([(s, sn.inverse(p)) for s, p in perm_repr])
#         perm_repr = perm_repr.union(
#             [(s1 * s2, sn.compose(p1, p2)) for s1, p1 in perm_repr for s2, p2 in perm_repr]
#         )
#         if len(perm_repr) == n:
...
#     new = IrrepsArray(irreps=tuple(MulRep(mul, ir) for mul, ir in new_irreps), list=new_list)
...
#     return IrrepsArray(irreps=tuple(MulRep(mul, ir) for mul, ir in new_irreps), list=new_list)
